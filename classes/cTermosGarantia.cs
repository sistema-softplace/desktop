/*
 * Projeto  : SoftPlace
 * Programa : cTermosGarantia - Termos de Garantia
...
namespace classes
{
	public class cTermosGarantia
	{
		public cTermosGarantia()
		{
		}
		...
		{
			string sql = "insert into TERMOS_GARANTIA values(" +
						 "'"  + codigo + "'," +
						 "'"  + descricao + "')";
		...
		{
			string sql = "update TERMOS_GARANTIA set " +
						 "DES_TERMO='" + descricao + "' " + 
		...
		{
			string sql = "delete from TERMOS_GARANTIA " + 
						 "where COD_TERMO='" + codigo + "'";
			FbCommand cmd = new FbCommand(sql, Globais.bd);
			try
			{

/*
 * Projeto  : SoftPlace
 * Programa : cPendenciasRecebimento - Pendencias de Recebimento
...
namespace classes
{
	public class cPendenciasRecebimento
	{
		public cPendenciasRecebimento()
		{
		}
		...
		{
			string sql = "insert into PENDENCIAS_RECEBIMENTO values(" +
						 "'"  + codigo + "'," +
						 "'"  + descricao + "'," +
						 "'"  + ativo + "')";			
		...
		{
			string sql = "update PENDENCIAS_RECEBIMENTO set " +
		...
		{
			string sql = "delete from PENDENCIAS_RECEBIMENTO " + 
						 "where COD_PENDENCIA='" + codigo + "'";
			FbCommand cmd = new FbCommand(sql, Globais.bd);
			try
			{

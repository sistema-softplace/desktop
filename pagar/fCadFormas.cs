 ...
 * Programa : fCadFormas - Cadastro de Formas de Pagamento
...
namespace pagar
{
	public partial class fCadFormas : tCadastroSimples
	{
		...
		void BtnConfirmaClick(object sender, EventArgs e)
		{
			...
			{
				if (acao == 'i')
					MessageBox.Show(codigo+"\n"+msg, "Erro na inclusão da forma de pagamento", MessageBoxButtons.OK, MessageBoxIcon.Error);
				else
					MessageBox.Show(codigo+"\n"+msg, "Erro na alteração da forma de pagamento", MessageBoxButtons.OK, MessageBoxIcon.Error);				
			}
		...
		void BtnExcluiClick(object sender, EventArgs e)
		{
			...
			if (!result)
			{
				MessageBox.Show(edtCodigo.Text, "Erro na exclusão da forma de pagamento", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return;
			}
			this.Cursor = Cursors.WaitCursor;
			formas.Carrega(dgvCadastro, chkAtivas.Checked);
